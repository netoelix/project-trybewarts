...
const LABEL_TEXTAREA = 'Descreva os principais pontos de aprendizado até aqui (1 hard skill / 1 soft skill):';
...
describe('Trybewarts', () => {
  ...
  describe('3 - Acrescente um formulário de login dentro do `header`, do lado direito', () => {
    ...
    it('Ao clicar no botão com email ou senha inválidos, emite um alerta contendo o texto "Email ou senha inválidos."', () => {
      const stub = cy.stub();
      cy.on('window:alert', stub);
      cy.get(`${TRYBEWARTS_LOGIN_FORM_SELECTOR} input[name=email]`).type('errado@teste.com');
      cy.get(`${TRYBEWARTS_LOGIN_FORM_SELECTOR} input[name=password]`).type('outrasenha');
      cy.get(`${TRYBEWARTS_LOGIN_FORM_SELECTOR} button`).click().then(() => {
  ...
  describe('13 - Crie 6 inputs do tipo `checkbox`', () => {
    ...
    const CHECKBOXES = ['Comunicação', 'Trabalho em equipe', 'Negociação', 'Escuta ativa', 'Métodos de array', 'Jest'];
  ...
  describe('15 - Crie um elemento `textarea`, de `id` `textarea` e número máximo de 500 caracteres', () => {
    it('Uma label com a classe textarea e o texto "Descreva os principais pontos de aprendizado até aqui (1 hard skill / 1 soft skill):" deverá ser criado" ', () => {
      cy.get('.textarea').contains(LABEL_TEXTAREA);
    });

    it('O campo textarea deverá permitir no máximo 500 caracteres', () => {
      cy.get('textarea')
        .clear()
        .type('text'.repeat(200));
      cy.get('textarea').invoke('val').should((value) => {
        expect(value).to.match(/^[a-z]{500}$/);
      });
    });

    it('O campo textarea deverá exigir o mínimo de 20 caracteres', () => {
      cy
        .get('textarea')
    ...
    it('O campo textarea deverá incluir a resposta para a pergunta', () => {
      cy.get('textarea').invoke('val').should((value) => {
